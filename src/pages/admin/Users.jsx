    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="w-full max-w-lg glass-panel rounded-2xl border border-slate-800 shadow-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-white">{user ? 'Edit User' : 'Create User'}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-lime-400" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-lime-400">
          ...
          {!user && (
            <div>
              ...
              <input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required minLength={8} className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-lime-400" />
            </div>
          )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-heading text-3xl font-black text-white">Users</h1>
          <p className="text-slate-400 text-sm mt-1">{users.length} system users with role-based access</p>
        </div>
                  ...
                  <tr key={u.id} className="hover:bg-slate-900/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setEditUser(u)} className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg" title="Edit"><Edit className="w-4 h-4" /></button>
                        <button onClick={() => setResetConfirm(u)} className="p-2 text-slate-400 hover:text-amber-400 hover:bg-amber-500/10 rounded-lg" title="Reset Password"><Key className="w-4 h-4" /></button>
                        <button onClick={() => setDeleteConfirm(u)} className="p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg" title="Delete"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-sm glass-panel rounded-2xl border border-slate-800 shadow-2xl p-6 text-center">
            ...
            <p className="text-slate-400 text-sm mb-6">Are you sure you want to delete {deleteConfirm.name}? This cannot be undone.</p>
      ...
      {resetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-sm glass-panel rounded-2xl border border-slate-800 shadow-2xl p-6 text-center">
            ...
            <p className="text-slate-400 text-sm mb-6">A temporary password will be generated for {resetConfirm.name}.</p>
            <div className="flex gap-3">
              <button onClick={() => setResetConfirm(null)} className="flex-1 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm font-semibold">Cancel</button>
      ...
      {resetResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-sm glass-panel rounded-2xl border border-slate-800 shadow-2xl p-6 text-center">
            ...
            <p className="text-slate-400 text-sm mb-2">Temporary password:</p>
            <code className="block p-3 rounded-xl bg-slate-900 border border-slate-700 text-lime-400 font-mono text-lg font-bold mb-4">{resetResult.tempPassword}</code>
            <p className="text-slate-500 text-xs mb-4">Share this password securely. The user will be forced to change it on next login.</p>
