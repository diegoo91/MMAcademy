...
export default function Imports() {
  ...
  const downloadTemplate = async () => {
    try {
      const res = await fetch(`http://localhost:5174/api/imports/template/${activeTab}`, { credentials: 'include' })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-3xl font-black text-white">Import Data</h1>
        <p className="text-slate-400 text-sm mt-1">Upload Excel files to bulk import players, results, or schedule slots</p>
      </div>
      ...
      <div className="flex gap-2">
        {tabs.map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === tab.id ? 'bg-lime-400 text-slate-950' : 'bg-slate-900 border border-slate-700 text-slate-300 hover:bg-slate-800'}`}
          >
            {tab.name}
          </button>
        ))}
        ...
        {result && (
          <div className="p-4 rounded-xl bg-lime-400/5 border border-lime-400/20 mb-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-white font-bold text-sm">Import Successful</span>
            </div>
            <p className="text-slate-400 text-xs">{result.inserted} {activeTab} imported from {result.totalRows} rows.</p>
          </div>
        )}
        ...
        {preview && !result && (
          <div>
            ...
            <div className="flex gap-3">
              <button onClick={reset} className="flex-1 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm font-semibold">Cancel</button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {batches.map(b => (
                  <tr key={b.id}>
                    <td className="py-3 text-white font-medium capitalize">{b.kind}</td>
                    <td className="py-3 text-slate-300">{b.filename}</td>
                    <td className="py-3 text-slate-300">{b.row_count}</td>
                    <td className="py-3 text-slate-300">{b.error_count}</td>
                    <td className="py-3 text-slate-300">{b.user_name || 'Unknown'}</td>
                    <td className="py-3">
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-lime-400/10 text-lime-400">{b.status}</span>
                    </td>
                    <td className="py-3 text-slate-400 text-xs">{new Date(b.created_at).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
