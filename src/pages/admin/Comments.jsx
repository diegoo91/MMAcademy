...
export default function Comments() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-3xl font-black text-slate-900 dark:text-white">Comments</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Review and moderate player comments</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-400 dark:text-slate-500 text-sm">No comments found.</div>
      ) : (
        <div className="space-y-3">
          {comments.map(c => (
            <div key={c.id} className="glass-panel rounded-2xl border border-slate-200 dark:border-slate-800 p-5 space-y-3">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                      <span className="text-sm font-bold text-slate-900 dark:text-white">{c.user_name || 'Unknown'}</span>
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 ml-2">{c.created_at?.slice(0, 16)}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-0.5 mb-2">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`w-3.5 h-3.5 ${i < (c.rating || 5) ? 'fill-amber-400 text-amber-400' : 'text-slate-300 dark:text-slate-700'}`} />
                    ))}
                  </div>
