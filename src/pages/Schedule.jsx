    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center space-y-3 mb-8">
          ...
          <h1 className="font-heading text-3xl sm:text-4xl font-black text-white">Academy Booking Schedule</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <button onClick={fetchSlots} className="px-4 py-2 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm font-semibold">Retry</button>
          </div>
        ) : (
          <>
            {dateChips.length > 0 && (
              <div className="mb-6 flex flex-wrap items-center justify-center gap-2">
                <span className="text-xs font-bold text-slate-400 uppercase mr-2">Quick Dates:</span>
                {dateChips.map((chip) => (
                  <button
                    key={chip.date}
            ...
            <div className="glass-panel rounded-3xl p-6 border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-2 bg-slate-900 p-1.5 rounded-2xl border border-slate-800">
                <button onClick={() => setScheduleView('day')} className={`px-5 py-2.5 rounded-xl font-extrabold text-xs transition-all flex items-center gap-2 ${scheduleView === 'day' ? 'bg-lime-400 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'}`}>
                  <CalendarIcon className="w-4 h-4" /><span>Day view (2 Courts)</span>
                </button>
                <button onClick={() => setScheduleView('week')} className={`px-5 py-2.5 rounded-xl font-extrabold text-xs transition-all flex items-center gap-2 ${scheduleView === 'week' ? 'bg-lime-400 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'}`}>
              ...
              {scheduleView === 'day' && (
                <div className="flex items-center gap-3">
                  ...
                  <input type="date" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)} className="px-4 py-2 rounded-xl bg-slate-900 border border-slate-700 text-white text-xs font-bold focus:outline-none focus:border-lime-400" />
                </div>
              )}

              <div className="flex items-center gap-4 text-xs font-bold">
                <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-lime-400" /><span className="text-slate-300">Available</span></div>
                <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-rose-500" /><span className="text-slate-300">Booked</span></div>
              </div>
            </div>

            {scheduleView === 'day' ? (
              <div className="glass-panel rounded-3xl p-6 border border-slate-800 mb-8">
                ...
                {currentDaySlots.length === 0 ? (
                  <p className="text-sm text-slate-400 py-8 text-center">No slots scheduled for this date — both courts are free all evening.</p>
                ) : (
                  <div className="overflow-hidden rounded-2xl border border-slate-800">
                    <div className="grid grid-cols-3 bg-slate-900/80 text-center">
                      <div className="px-4 py-3 text-xs font-extrabold uppercase tracking-wider text-slate-400 text-left">Time</div>
                      <div className="px-4 py-3 text-xs font-extrabold uppercase tracking-wider text-lime-400 border-l border-slate-800">Court 1</div>
                      <div className="px-4 py-3 text-xs font-extrabold uppercase tracking-wider text-lime-400 border-l border-slate-800">Court 2</div>
                    </div>
                    <div className="divide-y divide-slate-800/60">
                      {currentDaySlots.map((slot) => (
                        <div key={slot.time} className="grid grid-cols-3 items-stretch text-sm">
                          <div className="px-4 py-3 text-slate-300 font-mono text-xs flex items-center gap-1.5 bg-slate-900/40">
                            <Clock className="w-3.5 h-3.5 text-lime-400 shrink-0" />
                            {slot.label}
                          </div>
                          {[slot.court1, slot.court2].map((player, i) => (
                            <div key={i} className={`px-4 py-3 border-l border-slate-800/60 text-xs font-bold text-center flex items-center justify-center ${player ? 'bg-rose-500/10 text-rose-300' : 'bg-lime-400/5 text-lime-400'}`}>
                              {player || 'Available'}
                            </div>
                          ))}
                        </div>
              ...
              <div className="glass-panel rounded-3xl p-6 border border-slate-800 overflow-x-auto mb-8">
                <div className="min-w-[850px]">
                  <div className="grid grid-cols-6 gap-3 pb-4 border-b border-slate-800 text-center font-heading text-sm font-extrabold text-white">
                    <div className="text-left text-slate-400 text-xs uppercase">Time Slot</div>
                    {weekDates.map(date => (
                    ...
                    {weekTimes.map(time => (
                      <div key={time} className="grid grid-cols-6 gap-3 py-2 items-center text-xs">
                        ...
                        {weekDates.map(date => {
                          ...
                          const empty = !c1 && !c2
                          return (
                            <div key={date} className={`p-2.5 rounded-xl border text-[11px] font-bold text-center leading-snug ${empty ? 'bg-slate-900/40 text-slate-600 border-slate-800/40' : 'bg-rose-500/15 border-rose-500/40 text-rose-300'}`}>
                              {empty ? 'Available' : (
        ...
        <div className="p-6 rounded-3xl bg-gradient-to-r from-lime-500/10 via-slate-900 to-slate-900 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-3 rounded-2xl bg-lime-400 text-slate-950 font-bold"><CheckCircle2 className="w-6 h-6 stroke-[2.5]" /></div>
            <div>
              <h4 className="font-heading font-extrabold text-white text-base">Found an open slot?</h4>
              <p className="text-xs text-slate-400">Head to booking and lock in your 1-hour session.</p>
            </div>
          </div>
