...
export default function SignUp() {
  ...
  const inputClass = (hasError) =>
    `w-full pl-11 pr-4 py-3 rounded-xl bg-white/90 dark:bg-slate-900/90 border text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-sm focus:outline-none transition-all ${
      hasError
        ? 'border-rose-500 focus:ring-1 focus:ring-rose-500'
        : 'border-slate-300/80 dark:border-slate-700/80 focus:border-lime-400 focus:ring-1 focus:ring-lime-400'
    }`

  if (isSubmitted) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full glass-panel rounded-3xl p-8 text-center border border-slate-200 dark:border-slate-800 shadow-2xl relative overflow-hidden animate-fadeIn">
          ...
          <h2 className="font-heading text-3xl font-extrabold text-slate-900 dark:text-white mb-2">Welcome to MM Padel Academy!</h2>
          ...
          <div className="bg-white/90 dark:bg-slate-900/90 rounded-2xl p-4 mb-6 text-left border border-slate-200 dark:border-slate-800 text-xs space-y-2">
            ...
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">Selected Skill Level:</span>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <div className="text-center space-y-3 mb-10">
          ...
          <h1 className="font-heading text-3xl sm:text-4xl font-black text-slate-900 dark:text-white">Create Your Academy Account</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm max-w-md mx-auto">
            Get instant access to court reservations, coaching programs, and member sessions.
          </p>
        </div>

        <div className="glass-panel rounded-3xl p-6 sm:p-10 border border-slate-200/80 dark:border-slate-800/80 shadow-2xl">
          {serverError && (
          ...
          <form onSubmit={handleSubmit} className="space-y-6" noValidate>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="relative">
                  <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-slate-500" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300" aria-label="Toggle password visibility">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300" aria-label="Toggle confirm password visibility">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="font-extrabold text-slate-900 dark:text-white text-sm">{opt.name}</span>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-lime-400 border border-slate-300 dark:border-slate-700">{opt.rating}</span>
                      </div>
