...
export default function Payment() {
  ...
  const sessionLabel = sessionType === 'private' ? 'Private Coaching' : sessionType === 'group' ? 'Group Class' : ''
  ...
  if (bookingConfirmed) {
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 py-16 px-4 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="max-w-xl w-full glass-panel rounded-3xl p-8 border border-slate-800 shadow-2xl relative z-10 space-y-6 text-center animate-fadeIn">
          ...
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-lime-400/10 border border-lime-400/30 text-lime-400 text-xs font-bold uppercase tracking-wider mb-2">
              <Sparkles className="w-3.5 h-3.5" />
              <span>InstaPay Payment Verified</span>
            </div>
            <h1 className="font-heading text-3xl font-black text-white">Booking Confirmed!</h1>
            <p className="text-slate-300 text-sm mt-1">Show this reference upon arrival. Then send your payment screenshot on WhatsApp/phone to finalise.</p>
          </div>

          <div className="bg-slate-900/90 rounded-2xl p-6 border border-slate-800 text-left space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold block">Booking Reference ID</span>
                <span className="font-mono text-xl font-extrabold text-lime-400">{bookingReference}</span>
              </div>
              <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-lime-400 text-slate-950 uppercase">Confirmed Pass</span>
            </div>
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div><span className="text-slate-500 block">Session Category:</span><span className="text-white font-bold">{sessionLabel}</span></div>
              <div><span className="text-slate-500 block">Sessions:</span><span className="text-lime-400 font-bold">{sessionCount}</span></div>
              <div><span className="text-slate-500 block">Total Paid:</span><span className="text-lime-400 font-extrabold">{totalPrice.toLocaleString()} EGP</span></div>
              <div><span className="text-slate-500 block">Confirm on:</span><a href={CONTACT.phoneHref} className="text-slate-200 font-semibold hover:text-lime-400">{CONTACT.phone}</a></div>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button onClick={() => window.print()} className="flex-1 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs border border-slate-700 flex items-center justify-center gap-2 transition-all">
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="flex items-center justify-between mb-8">
          <button onClick={() => navigate('/book')} className="inline-flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-white transition-colors">
        ...
        {!booking ? (
          <div className="glass-panel rounded-3xl p-10 text-center border border-slate-800">
            <p className="text-slate-300 mb-6">No booking summary found. Please start a booking first.</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-7 glass-panel rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-2xl space-y-6">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-purple-600 text-white flex items-center justify-center font-black text-xs shadow-lg shadow-purple-600/30">IPN</div>
                  <div>
                    <h2 className="font-heading text-xl font-extrabold text-white">Pay via InstaPay</h2>
                    <p className="text-xs text-slate-400">Official National Instant Transfer</p>
                  </div>
                </div>
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-purple-950/40 border border-purple-500/30 space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-heading text-2xl font-black text-white mt-0.5">{totalPrice.toLocaleString()} <span className="text-sm font-bold text-lime-400">EGP</span></p>
                  </div>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950/90 border border-slate-800 flex items-center justify-between gap-3">
                  ...
                  <button onClick={handleCopyLink} type="button" className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold shrink-0 flex items-center gap-1.5 transition-colors">
            ...
            <div className="lg:col-span-5">
              <div className="glass-panel rounded-3xl p-6 border border-slate-800 shadow-2xl space-y-6">
                <div className="border-b border-slate-800 pb-4">
                  ...
                  <h3 className="font-heading text-lg font-extrabold text-white mt-0.5">Reservation Breakdown</h3>
                </div>

                <div className="space-y-4 text-xs">
                  <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-white text-sm">{sessionLabel}</span>
                    ...
                    <ul className="space-y-1.5 pt-2 border-t border-slate-800/60 max-h-56 overflow-y-auto pr-1">
                      {sessions.map((s, i) => (
                        <li key={i} className="text-slate-400">• {s.label}</li>
                      ))}
                    </ul>
                  </div>

                  <div className="pt-2 border-t border-slate-800 flex justify-between items-center">
                    <span className="font-extrabold text-white text-base">Total Amount Due</span>
                ...
                <div className="p-4 rounded-2xl bg-purple-950/20 border border-purple-500/30 text-[11px] text-slate-400 leading-relaxed">
